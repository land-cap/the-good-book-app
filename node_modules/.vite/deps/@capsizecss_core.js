...
// node_modules/.pnpm/@capsizecss+core@3.1.0/node_modules/@capsizecss/core/dist/capsizecss-core.browser.esm.js
...
var round = (value) => parseFloat(value.toFixed(4));
...
var _excluded$1 = ["::before", "::after"];
function createStyleString(ruleName, options) {
  const _createStyleObject2 = createStyleObject(options), {
    "::before": beforePseudo,
    "::after": afterPseudo
  } = _createStyleObject2, rootStyles = _objectWithoutProperties(_createStyleObject2, _excluded$1);
...
var toPercentString = (value) => "".concat(round(value * 100), "%");
var toCssProperty = (property) => property.replace(/([A-Z])/g, (property2) => "-".concat(property2.toLowerCase()));
var calculateOverrideValues = (_ref) => {
  let {
    metrics,
    fallbackMetrics
  } = _ref;
...
var quoteIfNeeded = (name) => {
  var _quotedMatch$groups;
  const quotedMatch = name.match(_wrapRegExp(/^['"](.*)['"]$/, {
    name: 1
  }));
...
var toCssString = (fontFaces) => {
  return fontFaces.map((_ref2) => {
    ...
    } = _ref2, restFontFaceProperties = _objectWithoutProperties(_ref2["@font-face"], _excluded);
...
function createFontStack(_ref3) {
  ...
  let optionsArg = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
  const {
    fontFaceFormat,
    fontFaceProperties
  } = _objectSpread2({
...
//# sourceMappingURL=@capsizecss_core.js.map
