...
function getDisplayName(name) {
  return utils.trimString(name, 20);
}
...
function createLocator(props) {
  const [enabledByPressingSignal, setEnabledByPressingSignal] = solidJs.createSignal(() => false);
  props.setLocatorEnabledSignal(solidJs.createMemo(() => enabledByPressingSignal()()));
  props.listenToDebuggerEenable((enabled) => {
    if (!enabled)
      setDevtoolsTarget(null);
  });
  const [hoverTarget, setHoverTarget] = solidJs.createSignal(null);
  const [devtoolsTarget, setDevtoolsTarget] = solidJs.createSignal(null);
  ...
  solidJs.createEffect(() => {
    if (!props.locatorEnabled())
      return;
    primitives.makeHoverElementListener((el) => setHoverTarget(el));
    solidJs.onCleanup(() => setHoverTarget(null));
...
function createDependencyGraph(props) {
  ...
  const triggerInspect = scheduled.throttle(inspectDGraph, 200);
...
var collectOwnerDetails = /* @__PURE__ */ primitives.untrackedCallback(function(owner, config) {
...
var walkSolidTree = /* @__PURE__ */ primitives.untrackedCallback(function(owner, config) {
  Mode = config.mode;
...
function createStructure(props) {
  ...
  const flushRootUpdateQueue = scheduled.throttle(forceFlushRootUpdateQueue, 250);
...
var plugin = createInternalRoot(() => {
  const hub = {
    output: eventBus.createGlobalEmitter(),
    input: eventBus.createGlobalEmitter()
  };
  const [modules, toggleModules] = utils$1.createStaticStore({
    debugger: false,
    locator: false,
    dgraph: false,
    locatorKeyPressSignal: () => false
  });
  const debuggerEnabled = solidJs.createMemo(() => modules.debugger || modules.locatorKeyPressSignal());
  const dgraphEnabled = solidJs.createMemo(() => modules.dgraph && debuggerEnabled());
  const locatorEnabled = solidJs.createMemo(
    () => (modules.locatorKeyPressSignal() || modules.locator) && debuggerEnabled()
  );
  const debuggerEnabledBus = eventBus.createEventBus();
  solidJs.createEffect(() => {
    if (!debuggerEnabled())
      debuggerEnabledBus.emit(false);
  });
  const viewChange = eventBus.createEventBus();
  ...
  const [inspectedState, setInspectedState] = solidJs.createSignal(INITIAL_INSPECTED_STATE, { equals: false });
  solidJs.createEffect(() => {
  ...
  solidJs.createComputed(
    utils$1.defer(debuggerEnabled, (enabled) => {
      if (!enabled)
        resetInspectedNode();
    })
  );
  ...
  solidJs.createEffect(
    utils$1.defer(modules.locatorKeyPressSignal, (state) => hub.output.emit("LocatorModeChange", state))
  );
  hub.input.listen((e) => {
    switch (e.name) {
      case "ResetState": {
